    ...
    public partial class Sales_Return : Form
    {
                        ...
                        if (dr.HasRows)
                        {
                            ...
                            if (((cartQty + userQty) < (salesQty - sales_return_qty)) || ((cartQty + userQty) == (salesQty - sales_return_qty)))
                            {
                                ...
                                if (dr4.HasRows)
                                {
                                    ...
                                    {
                                        dr5.Close();
                                        MessageBox.Show("This Product Already Exist in Sales Return Cart", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                    }
                                    else
                                    {
                                        dr5.Close();
                            ...
                            {
                                MessageBox.Show("Your Quantity Increases from Sales Invoice", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Question);
                            }
                        }
                        else
                        {
                            dr.Close();
                            MessageBox.Show("This Product is Not in the Sales Invoice", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        }
                    }
                    else
                    {
                        MessageBox.Show("Please Enter Sales Invoice No", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    Login.con.Close();
                }
            }
                            ...
                            if (((cartQty + userQty) < (salesQty - sales_return_qty)) || ((cartQty + userQty) == (salesQty - sales_return_qty)))
                            {
                                ...
                                if (dr.HasRows)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        txttotal.Text = Convert.ToString((Decimal)cmd10.ExecuteScalar() + (Decimal)cmd11.ExecuteScalar());
                                        txtgtotal.Text = Convert.ToString(Convert.ToDecimal(txttotal.Text) - Convert.ToDecimal(txtdeduct.Text));
                                        txtbalance.Text = Convert.ToString(Convert.ToDecimal(txtgtotal.Text) - Convert.ToDecimal(txtpaid.Text));
                                    }
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (txtinvoice.Text != "" && txtinvoice.Text != "0")
            {
                ...
                {
                    MessageBox.Show("You Have Not Enough Quantity Against This Product Code in Sales Invoice", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    String command = "select P_Code,Tone,Quality,Size,Boxes,Tiles,Meters,Price,Deduct,Amount from sr_cart";
                    test.bindingcode(command, dataGridView, Login.con);
                }
                Login.con.Close();
        ...
        private void ckbx2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (ckbx2.Checked == true)
            {
                txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = dataGridView2.Enabled = label10.Enabled = label5.Enabled = label4.Enabled = btnenter2.Enabled = true;
                cmbname.Focus();
            }
            else
                txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = dataGridView2.Enabled = label10.Enabled = label5.Enabled = label4.Enabled = btnenter2.Enabled = false;
            txtqty2.Text = "1";
            txtcode.Text = cmbname.Text = "";
            Login.con.Open();
            SqlCommand cmd2 = new SqlCommand("DELETE from sr_cart_s", Login.con);
            cmd2.ExecuteNonQuery();
            Login.con.Close();
            test.bindingcode("SELECT * From sr_cart_s", dataGridView2, Login.con);
        }
        ...
        private void txtinvoice_Leave(object sender, EventArgs e)
        {
            if (txtinvoice.Text != "")
            {
                if (Convert.ToDecimal(txtinvoice.Text) != 0)
                {
                    ...
                    {
                        MessageBox.Show("Please Enter Correct Invoice No", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        dr.Close();
                        txtinvoice.Clear();
                        txtinvoice.Focus();
                        cmb = false;
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (txtsinvoice.Text != "")
                {
                    if (Convert.ToDecimal(txtsinvoice.Text) != 0)
                    {
                        ...
                        {
                            dr.Close();
                            MessageBox.Show("Sales Return No Already Exist", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            txtsinvoice.Clear();
                            txtsinvoice.Focus();
                            sinvoice = false;
                            btnsubmit.Enabled = false;
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            Login.con.Close();
            DialogResult dresult = MessageBox.Show("Press 'OK' to Print SALES RETURN INVOICE", "Verona Tiles & Sanitary", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
