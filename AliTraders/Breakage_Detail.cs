...
namespace Verona
{
    public partial class Breakage_Detail : Form
    {
        ...
        private void Breakage_Detail_Load(object sender, EventArgs e)
        {
            ...
            string command2 = "select s.p_id P_Code,p.p_name P_Name,s.qty Quantity, ps.pprice Price, CAST(ROUND((s.qty * (ps.pprice - (ps.qty * ps.pprice - ps.discount)/ps.qty)), 2) as decimal(18,2)) Discount, CAST(ROUND((((s.qty) * (ps.pprice)) - (s.qty * (ps.pprice - (ps.qty * ps.pprice - ps.discount)/ps.qty))),2) as decimal(18,2)) Amount From breakage_detail2 s, breakage_stock bd, product2 p, purchase_stock_detail2 ps Where s.p_id=p.p_id AND ps.p_id=s.p_id AND ps.p_id=p.p_id AND ps.ps_id=bd.ps_id AND s.b_id=bd.b_id AND s.b_id ='" + invoice_no + "' AND bd.ps_id='" + Convert.ToDecimal(labeldc.Text) + "'";
