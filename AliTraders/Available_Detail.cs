...
namespace Verona
{
    public partial class Available_Detail : Form
    {
        ...
        private void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("create view total as select ps.ps_id, psd.p_id, psd.pprice,psd.qty, psd.discount ,CAST(ROUND((((psd.qty) * (psd.pprice)) - (psd.qty * (psd.pprice - (psd.qty * psd.pprice - psd.discount)/psd.qty))),2) as decimal(18,2)) total from purchase_stock_detail2 psd, purchase_stock ps, product2 p where psd.p_id=p.p_id AND ps.ps_id=psd.ps_id AND ps.datetime Between '" + Convert.ToDateTime(dateTimePicker1.Text) + "' AND '" + Convert.ToDateTime(dateTimePicker2.Text).AddDays(1) + "'", Login.con);
            cmd.ExecuteNonQuery();
            Login.con.Close();
        ...
        private void rdbtnavil2_CheckedChanged(object sender, EventArgs e)
        {
            if (rdbtnavil2.Checked == true)
            {
                ...
                SqlCommand cmd65 = new SqlCommand("IF EXISTS(select * FROM sys.views where name = 'total') DROP VIEW total", Login.con);
                cmd65.ExecuteNonQuery();
                SqlCommand cmd = new SqlCommand("create view total as select ps.ps_id, psd.p_id, psd.pprice,psd.qty, psd.discount ,CAST(ROUND((((psd.qty) * (psd.pprice)) - (psd.qty * (psd.pprice - (psd.qty * psd.pprice - psd.discount)/psd.qty))),2) as decimal(18,2)) total from purchase_stock_detail2 psd, purchase_stock ps, product2 p where psd.p_id=p.p_id AND ps.ps_id=psd.ps_id", Login.con);
                cmd.ExecuteNonQuery();
                Login.con.Close();
