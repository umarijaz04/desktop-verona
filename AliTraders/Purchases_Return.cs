    ...
    public partial class Purchases_Return : Form
    {
                        ...
                        if (dr.HasRows)
                        {
                            ...
                            if (((cartQty + userQty) < (salesQty - sales_return_qty)) || ((cartQty + userQty) == (salesQty - sales_return_qty)))
                            {
                                ...
                                if (dr4.HasRows)
                                {
                                    ...
                                    {
                                        dr5.Close();
                                        MessageBox.Show("This Product Already Exist in Purchases Return Cart", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                    }
                                    else
                                    {
                                        dr5.Close();
                            ...
                            {
                                MessageBox.Show("Your Quantity Increases from DC", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Question);
                            }
                        }
                        else
                        {
                            dr.Close();
                            MessageBox.Show("This Product is Not in the DC", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        }
                    }
                    else
                    {
                        MessageBox.Show("Please Enter DC No", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    Login.con.Close();
                }
            }
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (txtinvoice.Text != "" && txtinvoice.Text != "0")
            {
                ...
                {
                    MessageBox.Show("You Have Not Enough Quantity Against This Product Code in DC", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    String command = "select P_Code,Tone,Quality,Size,Boxes,Tiles,Meters,Price,Deduct,Amount from pr_cart";
                    test.bindingcode(command, dataGridView, Login.con);
                }
                Login.con.Close();
        ...
        private void ckbx2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            txtcode.Text = cmbname.Text = "";
            Login.con.Open();
            SqlCommand cmd2 = new SqlCommand("DELETE from pr_cart_s", Login.con);
            cmd2.ExecuteNonQuery();
            Login.con.Close();
            test.bindingcode("SELECT * From pr_cart_s", dataGridView2, Login.con);
        }
        ...
        private void txtinvoice_Leave(object sender, EventArgs e)
        {
            if (txtinvoice.Text != "")
            {
                if (Convert.ToDecimal(txtinvoice.Text) != 0)
                {
                    ...
                    {
                        MessageBox.Show("Please Enter Correct DC No", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        dr.Close();
                        txtinvoice.Clear();
                        txtinvoice.Focus();
                        cmb = false;
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (txtsinvoice.Text != "")
                {
                    if (Convert.ToDecimal(txtsinvoice.Text) != 0)
                    {
                        ...
                        {
                            dr.Close();
                            MessageBox.Show("Purchase Return No Already Exist", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            txtsinvoice.Clear();
                            txtsinvoice.Focus();
                            sinvoice = false;
                            btnsubmit.Enabled = false;
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            Login.con.Close();
            MessageBox.Show("Purchases Return Inserted Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
            reset_cart();
        }
