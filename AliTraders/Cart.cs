...
namespace Verona
{
    public partial class Cart : Form
    {
        ...
        {
            cmbcustomername.Text = txtsinvoiceno.Text = "";
            ...
            cmbquality2.Enabled = ckbx.Enabled = ckbx2.Enabled = cmbcustomername.Enabled = dateTimePicker.Enabled = txtsinvoiceno.Enabled = true;
        ...
        private void entercartdata()
        {
            if (ckbx.Checked == true)
            {
                if (lbname.Text != "" && txtprice.Text != "" && txtprice.Text != "0" && cmbtone.Text != ""  && cmbsize.Text != "" && cmbquality.Text != "" && (txtqty.Text != "0" || txttiles.Text != "0"))
                {
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        if (dr2.HasRows)
                        {
                            MessageBox.Show("This Product Already Exist in Sales Cart", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        }
                        else
                        {
                            dr2.Close();
                            if (Math.Round(user_qty,2) <= Math.Round(availableQty,2))
                            {
                                ...
                                if ((availableQty - (user_qty)) < limit)
                                    MessageBox.Show("Remain Product Quantity is '" + Convert.ToInt32((((availableQty - (user_qty)) * db_tiles) / db_tiles)) + " - " + Convert.ToInt32((((availableQty - (user_qty)) * db_tiles) % db_tiles)) + "'", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            ...
                            {
                                MessageBox.Show("You Have Not Enough Quantity Against This Product Code", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                txtqty.Focus();
                            }
                        }
                    }
            ...
            if (ckbx2.Checked == true)
            {
                if (txtprice2.Text != "")
                {
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        else
                        {
                            ...
                            if (user_qty <= availableQty)
                            {
                                ...
                                d.Close();
                                if ((availableQty - (user_qty)) < limit)
                                    MessageBox.Show("Remain Product Quantity is' " + (availableQty - user_qty) + "'", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            ...
                            {
                                txtqty2.Focus();
                                MessageBox.Show("You Have Not Enough Quantity Against This Product Code", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                    }
                    else
                    {
                        dr.Close();
                        MessageBox.Show("Record Not Found", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    Login.con.Close();
                }
            }
        ...
        {
            test.cmbox_drop2("SELECT o_id from order_stock", cmbdc, Login.con);
        }
        ...
        private void cmbdc_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd113 = new SqlCommand("SELECT o_id from order_detail2 where o_id = '" + cmbdc.Text + "'", Login.con);
            ...
            if (dr113.HasRows)
            {
                ...
                SqlDataAdapter a2 = new SqlDataAdapter("SELECT qty,p_id from order_detail2 where o_id='" + cmbdc.Text + "'", Login.con);
                ...
                foreach (DataRow row in dataTable2.Rows)
                {
                    ...
                    dr3.Close();
                    SqlCommand cmd2 = new SqlCommand("INSERT into cart_s values('" + row["p_id"].ToString() + "','" + p_name + "', '" + Convert.ToDecimal(row["qty"]) + "','0', '0', '0')", Login.con);
                    cmd2.ExecuteNonQuery();
                }
            }
            else
            ...
            SqlCommand cmd5 = new SqlCommand("SELECT m_id,c_id from order_stock where o_id='" + cmbdc.Text + "'", Login.con);
            SqlDataReader dr5 = cmd5.ExecuteReader();
            while (dr5.Read())
            {
            ...
            data = true;
            ckbx.Enabled = ckbx2.Enabled = cmbquality2.Enabled = txtmeter.Enabled = txtqty.Enabled = txttiles.Enabled = cmbcode.Enabled = cmbsize.Enabled = cmbquality.Enabled = cmbtone.Enabled = txtprice.Enabled = cmbcustomername.Enabled = txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = txtprice2.Enabled = false;
            dataGridView.Columns["Tiles"].ReadOnly = dataGridView.Columns["Boxes"].ReadOnly = dataGridView.Columns["P_Code"].ReadOnly = dataGridView.Columns["Tone"].ReadOnly = dataGridView.Columns["Quality"].ReadOnly = dataGridView.Columns["Size"].ReadOnly = dataGridView.Columns["Meter"].ReadOnly = dataGridView.Columns["Amount"].ReadOnly = dataGridView2.Columns["P_Code"].ReadOnly = dataGridView2.Columns["P_Name"].ReadOnly = dataGridView2.Columns["Amount"].ReadOnly = dataGridView2.Columns["Quantity"].ReadOnly = true;
        }
        ...
        private void txtmeter_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (cmbcode.Text != "" && cmbsize.Text != "")
                {
                    ...
                    MessageBox.Show("Please Enter Code & Size First", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
        }
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if ((p_qty + (p_tiles / Convert.ToDecimal(db_tiles))) <= availableQty)
            {
                ...
                if ((availableQty - (p_qty + (p_tiles / Convert.ToDecimal(db_tiles)))) < limit)
                    MessageBox.Show("Remain Product Quantity is '" +  Convert.ToInt32(((availableQty - (p_qty + (p_tiles / Convert.ToInt32(db_tiles))))*db_tiles)/db_tiles) + " - "+Convert.ToInt32(((availableQty - (p_qty + (p_tiles / Convert.ToInt32(db_tiles)))) * db_tiles) % db_tiles)+"'", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("You Have Not Enough Quantity Against This Product Code", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void dataGridView2_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (p_qty <= availableQty)
            {
                ...
                if ((availableQty - p_qty) < limit)
                    MessageBox.Show("Remain Product Quantity is' " + (availableQty - (p_qty)) + "'", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("You Have Not Enough Quantity Against This Product Code", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (txtsinvoiceno.Text != "")
                {
                    if (Convert.ToDecimal(txtsinvoiceno.Text) != 0)
                    {
                        ...
                        {
                            dr.Close();
                            MessageBox.Show("Sales Invoice No Already Exist", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            txtsinvoiceno.Clear();
                            txtsinvoiceno.Focus();
                            sinvoice = false;
                            btninvoice.Enabled = false;
        ...
        private void btninvoice_Click(object sender, EventArgs e)
        {
            ...
            dr1223.Close();
            if (flag == count && flag2 == count2)
            {
                MessageBox.Show("Please Set the Price in Cart First then Press 'Submit'", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Login.con.Close();
            }
            else if (flag == 0 && flag2 == 0)
            {
                ...
                Login.con.Close();
                //
                DialogResult dresult = MessageBox.Show("Press 'OK' to Print GATE PASS", "Verona Tiles & Sanitary", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                ...
                DialogResult dresult2 = MessageBox.Show("Press 'OK' to Print SALES INVOICE", "Verona Tiles & Sanitary", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
