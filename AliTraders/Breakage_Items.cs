    ...
    public partial class Breakage_Items : Form
    {
                        ...
                        if (dr.HasRows)
                        {
                            ...
                            if (((cartQty + (Convert.ToDecimal(txtqty.Text) + (Convert.ToDecimal(txttiles.Text) / Convert.ToDecimal(db_tiles)))) < (purchase_qty - breakage_qty)) || ((cartQty + (Convert.ToDecimal(txtqty.Text) + (Convert.ToDecimal(txttiles.Text) / Convert.ToDecimal(db_tiles)))) == (purchase_qty - breakage_qty)))
                            {
                                ...
                                if (dr4.HasRows)
                                {
                                    ...
                                    {
                                        dr5.Close();
                                        MessageBox.Show("This Product Already Exist in Breakage Cart", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                    }
                                    else
                                    {
                                        dr5.Close();
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (txtdcno.Text != "" && txtdcno.Text != "0")
            {
                ...
                {
                    MessageBox.Show("You Have Not Enough Quantity Against This Product Code in Purchases Invoice", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    String command = "select P_Code,Tone,Quality,Size,Boxes,Tiles,Meters from breakage_cart";
                    test.bindingcode(command, dataGridView, Login.con);
                }
                Login.con.Close();
        ...
        private void ckbx2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (ckbx2.Checked == true)
            {
                txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = dataGridView2.Enabled = label9.Enabled = label8.Enabled = label7.Enabled = btnenter2.Enabled = true;
                cmbname.Focus();
            }
            else
                txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = dataGridView2.Enabled = label9.Enabled = label8.Enabled = label7.Enabled = btnenter2.Enabled = false;
            txtqty2.Text = "1";
            txtcode.Text = cmbname.Text = "";
            Login.con.Open();
            SqlCommand cmd2 = new SqlCommand("DELETE from breakage_cart_s", Login.con);
            cmd2.ExecuteNonQuery();
            Login.con.Close();
            test.bindingcode("SELECT * From breakage_cart_s", dataGridView2, Login.con);
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            Login.con.Close();
            MessageBox.Show("Breakage Inserted Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
            cart_clear();
        }
