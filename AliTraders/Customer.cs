...
namespace Verona
{
    public partial class Customer : Form
    {
        ...
        private void btnadd_Click(object sender, EventArgs e)
        {
            if (txtnameadd.Text != "")
            {
                ...
                else
                {
                    ...
                    MessageBox.Show("Customer Inserted Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Login.con.Close();
                    addToolStripMenuItem_Click(this, null);
                }
                Login.con.Close();
            ...
            if (txtcodeup.Text != "" && txtnameup.Text != "")
            {
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        if ((txtnameup.Text.ToUpper() == Convert.ToString(dr[0])))
                        {
                            flag = false;
                            dr.Close();
                            MessageBox.Show("This Customer is Already Exist", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            break;
                        }
                    }
                }
                if (flag)
                {
                    ...
                    try
                    {
                        ...
                        MessageBox.Show("Customer Updated Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        updateToolStripMenuItem_Click(this, null);
                    }
                    catch (Exception)
                    {
                        MessageBox.Show("This Customer is Already Used in Transaction", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    }
                }
                Login.con.Close();
            }
        ...
        private void btndel_Click(object sender, EventArgs e)
        {
            if (delid != "")
            {
                DialogResult dresult = MessageBox.Show("Are you sure you want to delete this Customer", "Verona Tiles & Sanitary", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (dresult == DialogResult.OK)
                {
                    try
                    {
                        ...
                        delid = "";
                        MessageBox.Show("Customer Deleted Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    catch (Exception)
                    {
                        MessageBox.Show("This Customer is Already Used in Transaction", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
