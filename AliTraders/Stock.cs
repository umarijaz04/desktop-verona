...
namespace Verona
{
    public partial class Stock : Form
    {
        ...
        void cart_clear()
        {
            cmbmanu.Text = txtdcno.Text = "";
            cmbquality2.SelectedIndex = 1;
            txttotal.Text = txtpaid.Text = txtbalance.Text = txtprebalance.Text = "0";
            cmbquality2.Enabled = ckbx.Enabled = ckbx2.Enabled = cmbmanu.Enabled = dateTimePicker.Enabled = txtdcno.Enabled = true;
        ...
        private void entercartdata()
        {
            ...
            if (ckbx.Checked == true)
            {
                if (txttone.Text != "" && cmbsize.Text != "" && (txtqty.Text != "0" || txttiles.Text != "0"))
                {
                    ...
                    if (dr.HasRows)
                    {
                        MessageBox.Show("This Product Already Exist in Purchases Cart", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    }
                    else
                    {
                        dr.Close();
        ...
        {
            test.cmbox_drop2("SELECT ps_id from purchase_stock where pending='YES'", cmbdc, Login.con);
        }
        ...
        private void cmbdc_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dr113.HasRows)
            {
                ...
                SqlDataAdapter a2 = new SqlDataAdapter("SELECT qty,p_id from purchase_stock_detail2 where ps_id='" + cmbdc.Text + "'", Login.con);
                ...
                foreach (DataRow row in dataTable2.Rows)
                {
                    ...
                    dr3.Close();
                    SqlCommand cmd2 = new SqlCommand("INSERT into purchase_cart_s values('" + row["p_id"].ToString() + "','" + p_name + "', '" + Convert.ToDecimal(row["qty"]) + "','0', '0', '0')", Login.con);
                    cmd2.ExecuteNonQuery();
                }
            }
            else
            ...
            ckbx.Enabled = ckbx2.Enabled = txtqty.Enabled = txttiles.Enabled = cmbcode.Enabled = cmbsize.Enabled = cmbquality.Enabled = txttone.Enabled = txtprice.Enabled = cmbmanu.Enabled = cmbquality2.Enabled = txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = txtprice2.Enabled = false;
            btnsubmit.Enabled = dataGridView.Columns["Tiles"].ReadOnly = dataGridView.Columns["Boxes"].ReadOnly = dataGridView.Columns["P_Code"].ReadOnly = dataGridView.Columns["Tone"].ReadOnly = dataGridView.Columns["Quality"].ReadOnly = dataGridView.Columns["Size"].ReadOnly = dataGridView.Columns["Meters"].ReadOnly = dataGridView.Columns["Amount"].ReadOnly = dataGridView2.Columns["P_Code"].ReadOnly = dataGridView2.Columns["P_Name"].ReadOnly = dataGridView2.Columns["Amount"].ReadOnly = dataGridView2.Columns["Quantity"].ReadOnly = true;
        }
        ...
        private void rdbtn2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (ckbx2.Checked == true)
            {
                ...
                txtqty2.Enabled = txtcode.Enabled = cmbname.Enabled = txtprice2.Enabled = dataGridView2.Enabled = label21.Enabled = label20.Enabled = label22.Enabled = label23.Enabled = false;
            txtqty2.Text = "1";
            ...
            { 
                if (txtdcno.Text != "")
                {
                    if (Convert.ToDecimal(txtdcno.Text) != 0)
                    {
                        ...
                        {
                            dr.Close();
                            MessageBox.Show("DC No Already Exist", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            txtdcno.Clear();
                            txtdcno.Focus();
                            dc = false;
                            btnsubmit.Enabled = false;
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            if (flag == count && flag2 == count2)
            {
                if (txtdcno.Enabled != false)
                {
                    ...
                    MessageBox.Show("Stock Inserted Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Login.con.Close();
                    cart_clear();
                }
                else
            ...
            else if (flag == 0 && flag2 == 0)
            {
                if (txtpaid.Text == "0")
                {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Stock Updated Successfully", "Verona Tiles & Sanitary", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Login.con.Close();
                        cart_clear();
                    }
                }
